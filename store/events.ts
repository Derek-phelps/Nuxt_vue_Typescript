...
export const actions: ActionTree<EventsState, RootState> = {
		...
		return this.$api.$get('events/', { params }).then((data) => {
			...
			if (Array.isArray(events)) {
				data = events.map((event) => {
					if (event.location) {
						ctx.commit('locations/add', event.location, { root: true })
					}

					if (event.categories) {
						ctx.commit('categories/add', event.categories, { root: true })
					}

					return event
				})

				ctx.commit('add', events)
			} else throw data

			return data.filter((i: any) => i)
		})
	},
	fetch_single(ctx, eventId: number) {
		const existing = ctx.state.items[eventId]
		return new Promise((resolve, reject) => {
			// if (existing) resolve(ctx.state.items[eventId])
	...
	fetch_guests(ctx, eventId: number) {
		return this.$api.$get(`events/${eventId}/guests/`)
	},
}
