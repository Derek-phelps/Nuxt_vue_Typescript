...
import { ClubReservations } from '@store/reservations'
import _ from 'lodash'
import { Vue, Component, InjectReactive, Getter } from 'nuxt-property-decorator'

@Component
export default class ReservationStepGuestQuestions extends ClubReservations {
	...
	get required_questions() {
		return this.club_reservation_questions('guests').filter(
			(f) => f.is_required
		)
	}
	...
	all_required_questions_answered() {
		...
		this.club_reservation.guests.forEach((g) =>
			questions.push(...g.form_answers)
		)
	...
	updated() {
		console.log(this.required_questions_answered())
	}
...
<template>
	<div class="step-page step-3">
		<div class="question">
			<h2 class="step-title">
				Please answer the following question(s) for this attendee ...
			</h2>
