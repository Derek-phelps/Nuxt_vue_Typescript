...
.form-search {
	...
	&:after {
		content: ' ';
		width: calc(
			var(--results-width, 100%) + calc(1em - #{$scrollbar-size-var})
		);
		height: calc(var(--search-height, 0px) + 2em);

		position: fixed;
		left: calc(var(--results-x, 0px) - 0.5em);
		top: calc(var(--results-y, 100%) - calc(var(--search-height, 0.5em) + 1em));
		z-index: 21;
	...
	.search-results-wrap {
		width: calc(var(--results-width, 100%) + 1em);
		height: calc(var(--search-height, 0px) + 2em);
